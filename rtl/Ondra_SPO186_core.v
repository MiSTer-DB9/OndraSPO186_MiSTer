...
(
   ...
   input  wire  [4:0] joystick,      // FIRE DOWN UP LEFT RIGHT

   output wire        clk_video,
   output reg         HSync,         // Horizontal Sync  ... 0 = sync pulse
   output reg         VSync,         // Vertical   Sync  ... 0 = sync pulse
   output wire        HBlank,        // Horizontal Blank ... 0 = active row,    1 = Blank
   output wire        VBlank,        // Vertical   Blank ... 0 = active screen, 1 = Blank
   output reg         pixel,
   output wire        beeper,
   ...
   output reg         MGF_OUT,      // cassette line out
...
//----------------------- VIDEO -----------------------------------
//
// great page explaining Ondra's video: https://sites.google.com/site/ondraspo186/4-grafika/4-1-ako-to-funguje?authuser=0
//
...
// **** MiSTer VBlank fix ****
assign  VBlank = (VBlank_cnt == 8'd0) & Vcnt_out0;
reg [7:0] VBlank_cnt;
reg Hcnt_out1_last2;
reg Vcnt_out0_last2;
...
always @(posedge clk_sys)
begin
   Vcnt_out0_last2 <= Vcnt_out0;
   Hcnt_out1_last2 <= Hcnt_out1;

   if ((Hcnt_out1_last2 & ~Hcnt_out1) & ~(VBlank_cnt == 8'd0))
      VBlank_cnt <= VBlank_cnt - 1'b1;
   else if (Vcnt_out0_last2 & ~Vcnt_out0)
      VBlank_cnt <= 8'd255;
end
// **** MiSTer VBlank fix end ****
...
begin
   ...
   if (~videoOff_n | ~VSync)
      BUSRQ_n <= 1;
   else if (Vcnt_out2 & ~Vcnt_out2_last) // UP
      BUSRQ_n <= 0;
   else if (Vcnt_out0 & ~Vcnt_out0_last) //  DOWN
      BUSRQ_n <= 1;
end
...
wire [4:0] row_KJ             = (A[3:0] == 4'b1001) ? joystick : row_Keyboard;
...
wire [11:0] EPROM_Addr_4k  = { ROM1_en, A[10:0] };

// if 2x2764 EPROM chip used
wire [13:0] EPROM_Addr_16k = { ROM1_en, A[12:0] };


wire [7:0] data_EPROM0_out;  // OndraViLi_v27
wire [7:0] data_EPROM1_out;  // Ondra_TESLA_V5
wire [7:0] data_EPROM2_out;  // Ondra_PLUS 1.4
wire [7:0] data_EPROM3_out;  // Ondra_test
...
dpram #(.ADDRWIDTH(12), .MEM_INIT_FILE("./ROM/OndraViLi_v27.mif")) myEPPROM0
(
   .clock(clk_sys),
   .address_a(EPROM_Addr_4k),
   .wren_a(0),
   .q_a(data_EPROM0_out)
);


dpram #(.ADDRWIDTH(12), .MEM_INIT_FILE("./ROM/Ondra_TESLA_V5.mif")) myEPPROM1
(
   .clock(clk_sys),
   .address_a(EPROM_Addr_4k),
   .wren_a(0),
...
dpram #(.ADDRWIDTH(14), .MEM_INIT_FILE("./ROM/Ondra_PLUS_1_4.mif")) myEPPROM2
(
   .clock(clk_sys),
   .address_a(EPROM_Addr_16k),
   .wren_a(0),
   .q_a(data_EPROM2_out)
);


dpram #(.ADDRWIDTH(10), .MEM_INIT_FILE("./ROM/Ondra_test.mif")) myEPPROM3
(
   .clock(clk_sys),
   .address_a(EPROM_Addr_4k),
   .wren_a(0),
